...
class ParserSwagger(object):
    ...
    def __init__(self, swagger_path):
        ...
        self.tags = {}
        self.build_tags()
        self.paths = list(self.specification.get('paths', {}).keys())
        self.definitions = list(self.specification.get('definitions', {}).keys())
        self.paths_data = {}
    ...
    def build_tags(self):
        for tag_dict in self.specification.get('tags', []):
            ...
            if tag:
                self.tags.update({tag: tag_dict.get('description', '')})
...
if __name__ == '__main__':
    from Config.directory import yaml_dir
    import os

    filename = os.path.join(yaml_dir, "exam-swagger.yaml")
    p = ParserSwagger(filename)
    # print (p.paths_data['/users/phone'])

    print(p.specification['paths']['/users/phone'])
    print(p.paths_data['/users/phone'])
